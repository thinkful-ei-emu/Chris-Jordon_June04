...
function max(numbers) {
  ...
  while (i < numbers.length) {
    if(numbers[i] > max) {
      max=numbers[i];// replace numbers to become the new wmax
    }
    i++; //increase on array to try new number
  }
...
function atestFunctionWorks(fn, input, expected) {
  if (fn(input) === expected) {
    console.log('SUCCESS: `' + fn.name + '` works on `[' + input + ']`');
    return true;
  } else {
...
(function runTests() {
  ...
  const testResults = [
    atestFunctionWorks(average, numList1, correctAns1),
    atestFunctionWorks(average, numList2, correctAns2),
  ];
  const numPassing = testResults.filter(function(result) {
    return result;
  }).length;
